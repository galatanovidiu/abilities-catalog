<?php
/**
 * Shared MCP resources and prompt for the catalog servers.
 *
...
/**
 * Builds the read-only discovery content shared by both catalog servers.
 *
 * @since 0.5.0
 */
final class DiscoveryContentFactory {
	...
	/**
	 * Builds the shared resources.
	 *
	 * @param callable $permission The server's coarse authentication floor.
	 * @return list<\WP\MCP\Domain\Resources\McpResource> The resources that were built successfully.
	 */
	 ...
	 *
	 * @param callable $permission The server's coarse authentication floor.
	 * @return list<\WP\MCP\Domain\Prompts\McpPrompt> The prompts that were built successfully.
	 */
	public static function prompts( callable $permission ): array {
		$prompt = McpPrompt::fromArray(
			array(
				...
				'description' => __( 'Builds a discovery-first workflow for a WordPress task: find the matching ability, inspect its exact schema and safety annotations, then execute only when authorized.', 'abilities-catalog' ),
				'arguments'   => array(
					array(
						'name'        => 'task',
						...
						'messages'    => array(
							array(
								...
								'content' => array(
									'type' => 'text',
									'text' => sprintf(
										/* translators: %s: WordPress task supplied to the prompt. */
