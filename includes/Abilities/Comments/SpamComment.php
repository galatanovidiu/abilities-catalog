 ...
 * `spam`. Takes only the comment ID. The `permission_callback` encodes the
 * catalog capability: `moderate_comments` OR object-level `edit_comment`. The
 * REST route re-checks the capability and applies the status change.
 *
 * @since 0.2.0
 */
final class SpamComment implements Ability {
	...
	public function args(): array {
		return array(
			...
			'description'         => __( 'Marks a comment as spam, setting its status to "spam". Requires moderate_comments or edit permission on the comment.', 'abilities-catalog' ),
			'category'            => 'comments',
			'input_schema'        => array(
				'type'                 => 'object',
				'properties'           => array(
