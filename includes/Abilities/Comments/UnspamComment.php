 ...
 * `unspam`. The controller restores the comment to its prior status (typically
 * `approved` or `hold`). Takes only the comment ID. The `permission_callback`
 * encodes the catalog capability: `moderate_comments` OR object-level
 * `edit_comment`. The REST route re-checks the capability and applies the
 * status change.
 *
 * @since 0.2.0
 */
final class UnspamComment implements Ability
{
	...
	public function args(): array
	{
		return array(
			...
			'description'         => __('Removes the spam mark from a comment, restoring its prior status. Requires moderate_comments or edit permission on the comment.', 'abilities-catalog'),
			'category'            => 'comments',
			'input_schema'        => array(
				'type'                 => 'object',
				'properties'           => array(
