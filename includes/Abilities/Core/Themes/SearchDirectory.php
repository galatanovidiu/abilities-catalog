...
final class SearchDirectory implements Ability {
		...
		return array(
			...
			'output_schema'       => array(
				...
				'properties'           => array(
					...
					'total_results' => array(
						'type'        => 'integer',
						'description' => __( 'The total number of matching themes across all pages.', 'abilities-catalog' ),
					),
					'has_more'      => array(
						'type'        => 'boolean',
	...
	public function execute( $input ) {
		...
		if ( is_wp_error( $result ) ) {
			// Preserve the stable core error code (e.g. `themes_api_failed`).
			// themes_api() returns its error with the raw remote body (a string) as
