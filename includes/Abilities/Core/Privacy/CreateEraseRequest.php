 ...
 * mirrors the wp-admin Erase Personal Data screen, which gates on both
 * `erase_others_personal_data` and `delete_users`
 * (wp-admin/erase-personal-data.php:12).
 *
 * @since 0.3.0
 */
final class CreateEraseRequest implements Ability {
		...
		return array(
			...
			'input_schema'        => array(
				...
				'properties'           => array(
					'email'                   => array(
						'type'        => 'string',
						'format'      => 'email',
						'description' => __( 'The email address of the data subject to erase.', 'abilities-catalog' ),
					),
			...
			'output_schema'       => array(
				...
				'properties'           => array(
					...
					'action_name' => array(
						'type'        => 'string',
						'description' => __( 'The request action name (remove_personal_data).', 'abilities-catalog' ),
					),
				),
				'additionalProperties' => false,
			),
	 ...
	 * `erase_others_personal_data` and `delete_users`. The wrapped core function
	 * `wp_create_user_request()` performs no capability check, so this is the
	 * hard authorization guard.
	 *
	 * @param mixed $input The validated input data.
	 * @return bool True if the current user may create an erase request.
	 */
	...
	/**
	 * Executes the ability by creating an erase request.
	 *
	 * @param mixed $input The validated input data.
