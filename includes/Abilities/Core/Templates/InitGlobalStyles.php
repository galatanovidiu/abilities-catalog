...
/**
 * T1 write ability: `templates/init-global-styles`.
 *
 ...
 * is reachable: call it, then pass the returned id to `templates/update-global-styles`.
 *
...
final class InitGlobalStyles implements Ability {
	...
	public function name(): string {
		return 'templates/init-global-styles';
	}
	...
	public function args(): array {
		return array(
			...
			'description'         => __( 'Ensures the active theme has a user global-styles record and returns its post ID, creating an empty record if none exists yet. Idempotent: returns the same ID on repeat calls. Call this before templates/update-global-styles when templates/get-global-styles reports no record (404) — for example to change colors or fonts on a freshly set up block theme.', 'abilities-catalog' ),
			'category'            => 'templates',
			'input_schema'        => array(),
			'output_schema'       => array(
				'type'                 => 'object',
				...
				'properties'           => array(
					'id' => array(
						'type'        => 'integer',
						'description' => __( 'The user global-styles post ID for the active theme (pass it to templates/get-global-styles or templates/update-global-styles).', 'abilities-catalog' ),
					),
				),
