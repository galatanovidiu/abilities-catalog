...
/**
 * Central catalog of ability categories.
 *
 * One descriptor per domain, keyed by slug. The {@see Registry} registers each
 * on `wp_abilities_api_categories_init`; every ability references its category
 * through `args()['category']` (the slug), so a slug used by an ability MUST
 * exist here.
 *
 * Labels and descriptions call `__()`, so {@see all()} must be invoked at or
 * after the relevant init hook (when translations are available), never at
 * file load.
 *
 * @since 0.3.0
 */
...
{
	/**
	 * The category descriptors, keyed by slug.
	 *
