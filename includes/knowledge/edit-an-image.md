...
- media execute og-media/list-media to find the image by title or filename, or og-media/get-media to read one attachment by ID. Both return the attachment ID and its file URL (source_url) — you need that URL in Step 2A.

STEP 2A - CHANGE THE PIXELS: ROTATE, CROP, FLIP (through the "media" tool)
- media execute og-media/edit-media-image. It needs BOTH the attachment "id" AND "src" — the URL of the image file to edit, read from og-media/get-media in Step 1. The id alone is not enough.
...
- media execute og-media/update-media. This sets alt text, title, caption, or description — it does NOT alter the image pixels. Use it for accessibility alt text; use Step 2A to rotate or crop.

STEP 3 - REBUILD THE SIZES (through the "media" tool)
- media execute og-media/regenerate-thumbnails to rebuild an attachment's sub-sizes — useful when the registered image sizes have changed or a derivative is missing. A simple edit in Step 2A already returns a new attachment with its sizes generated, so you do not need this after it. og-media/list-image-sizes shows which sizes exist.

To work on a brand-new image, media execute og-media/upload-media first, then return to Step 2.
