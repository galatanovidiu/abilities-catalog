<?php
/**
 * Integration tests for the og-plugins/search-directory ability.
 *
 ...
 * error-preservation contract for the og-plugins/search-directory ability.
 *
...
final class SearchDirectoryTest extends TestCase {
	...
	public function test_ability_is_registered(): void {
		$this->assertNotNull( wp_get_ability( 'og-plugins/search-directory' ) );
	}
	...
	public function test_returns_shaped_items_and_pagination_for_admin(): void {
		...
		$result = wp_get_ability( 'og-plugins/search-directory' )->execute( array( 'search' => 'spam' ) );
	...
	public function test_has_more_is_false_when_page_covers_all_results(): void {
		...
		$result = wp_get_ability( 'og-plugins/search-directory' )->execute(
			array(
				'search'   => 'spam',
				'page'     => 1,
				'per_page' => 10,
	...
	public function test_wp_error_with_raw_body_data_gets_status_and_hides_body(): void {
		$this->actingAs( 'administrator' );
		// A real plugins_api() transport failure carries the raw remote body (a
		...
		$this->stubPluginsApi( new WP_Error( 'plugins_api_failed', 'An unexpected error occurred.', '<html>500 Internal Server Error</html>' ) );
	...
	public function test_output_schema_declares_pagination_fields(): void {
		$schema = wp_get_ability( 'og-plugins/search-directory' )->get_output_schema();
