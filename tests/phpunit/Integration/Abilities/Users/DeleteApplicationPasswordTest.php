 ...
 * a deletion result carrying a snapshot of the revoked credential out. Adapter-backed:
 * permission delegates to the wrapped DELETE route, which now runs at dispatch, so a
 * denial surfaces through execute() as the route's REAL error (e.g.
 * rest_cannot_delete_application_password 403), not the generic ability_invalid_permissions
 * collapse. Locks the output shape (name and app_id, not just deleted + uuid) so an agent
 * can confirm which credential was revoked, and proves an unknown uuid surfaces core's
...
final class DeleteApplicationPasswordTest extends TestCase {
	...
	public function test_wrong_capability_surfaces_specific_403_and_credential_survives(): void {
		// An author lacks edit_user on another user's credential. The ability sets no
		// require_permission floor, so the wrapped DELETE route is the authority: its own
		// delete guard runs at dispatch and denies the revoke with its specific 403 —
		// surfaced through execute() unchanged, not collapsed to ability_invalid_permissions.
		// The credential is untouched, proving the object-level guard still holds.
