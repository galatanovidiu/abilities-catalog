 ...
 * shaped profile out, with the login guard enforced by the wrapped REST route on
...
final class GetCurrentUserTest extends TestCase {
	...
	public function test_logged_out_user_is_denied(): void {
		// Adapter-backed: permission delegates to the wrapped route. `/wp/v2/users/me`
		// requires a logged-in user, and the route's check now runs at dispatch, so
		// execute() surfaces the route's REAL error — not the generic collapse. This
		// preserves the catalog's old login floor without a require_permission guard.
